...
import serviceData from "@/public/documents/sercices.json"
import { fetchBaseServiceQuery, useFetchPostionsQuery } from '@/app/hooks/services.hook';
...
const Services = () => {
    ...
    const handleRedirect = async (id: number) => {
        ...
        const expires: any = session?.expires;
        const isTokenExpired = new Date(expires) < new Date();
        if (!token) {
            ...
            window.location.href = `/serve/book-service?position=${id}`;
        }
    };
            ...
            <div className='w-full flex flex-col relative bg-[#FAFAFA] '>
                ...
                {isLoading ? (
                                ...
                                <div key={index} id={item?.position_title} className={`flex flex-col ${index % 2 !== 0 ? "lg:flex-row-reverse" : "lg:flex-row"} lg:flex-row w-full gap-[20px] items-center py-20`}>
                                    <div className='w-full lg:w-1/2 flex flex-col gap-[10px]'>
                                        ...
                                        <div className='grid grid-cols-2  gap-[20px] md:pr-10'>
                                            <div onClick={() => handleRedirect(item?.id)} className=' cursor-pointer hover:opacity-90 flex flex-row items-center justify-center rounded-[30px] p-[15px] bg-[#13829F]'>
                                                ...
                                                <div className='absolute z-20 p-3 bg-white rounded-[20px] w-[90%] mx-auto flex left-1/2 bottom-3  transform -translate-x-1/2 '>
                                                    <div className='p-3 bg-[#F8F8F8] w-full rounded-[20px] flex flex-row gap-[4px]'>
                                                        ...
                                                        <div className='flex flex-col gap-[3px]'>
                                                            ...
                                                            <h1 className='text-[18px] font-[800]'>${item?.selling_price}</h1>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    {positions?.data.length === 0 && (
                        <div className='py-40 items-center  justify-center flex flex-col gap-[4px]'>
                            <span className='text-[18px] font-[900] text-black'>No Service yet</span>
                            <span className='text-[14px] text-[#4b4b4b]'>You will get back soon Thanks!!</span>
                        </div>
                    )}
                </>)}
            </div>
            <Dialog header={`Loggin Alert`} className='w-[90%] md:w-1/3' visible={!isLoggedIn} onHide={() => setIsLoggedIn(true)}>
                <div className="flex flex-col items-center justify-center gap-[10px]">
                    ...
                    <div className='flex flex-row items-center gap-[10px]'>
                        <a href='/auth' className='px-10 py-3 rounded-[6px] text-[13px] bg-primary  text-white'>
                            Sign In
                        </a>
                        <a href='/auth/singup' className='px-10 py-3 rounded-[6px] text-[13px] border boder-primary  text-primary'>
                            Sign Up
                        </a>
                    </div>
                </div>
