...
import { fetchBaseServiceQuery } from '../hooks/services.hook';
  ...
  return (
    <div className="flex flex-col bg-[#FFFFFF]">
      ...
      <div className='flex flex-col gap-[10px] px-[10px] md:px-[50px] lg:px-[100px] py-4 '>
        <span className='text-[16px] font-[600] text-black'>Service categories</span>
        <div className='grid grid-cols-3 gap-[20px] '>
          ...
          {services.map((service:any,index: number) => {
            return (
              <a href='./service-company' key={index} className='flex relative cursor-pointer hover:opacity-70 overflow-hidden duration-300 flex-col justify-between p-4 rounded-[24px] bg-[#F6F6F6] h-[25vh]'>
                <div className='z-10 absolute w-full h-full top-0 left-0'>
                  <Image src={`/image/homeclean.png`} width={1000} height={1000} alt='images'/>
                </div>
                <div></div>
                <div className='flex flex-col p-3 relative z-20 px-6 bg-white rounded-[12px]'>
                  <h1 className='text-[16px] font-[700] text-black'>{service.title}</h1>
                  <span className='text-[13px] font-[400] line-clamp-1 text-[#939191]'>{service.description}</span>
                </div>
              </a>
            )
          })}
